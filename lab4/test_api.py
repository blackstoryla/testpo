...
@pytest.mark.parametrize("animal_type", ["", "-", "Человек"])
@pytest.mark.parametrize("age", [-1, 100])
def test_add_new_pet_not_valid_all(auth_key, name, animal_type, age):
    auth_key = {'key': auth_key}
...
@pytest.mark.parametrize("animal_type", ["", "-", "Человек"])
@pytest.mark.parametrize("age", [-1, 100])
def test_update_pet_info_not_valid_all(auth_key, pet_id, name, animal_type, age):
    auth_key = {'key': auth_key}
...
@pytest.mark.parametrize("auth_key", ["abracadabra", "e2c4926695071b220431a96f3a1ccd9c0f9e6e0e43e2e0aec70ad1e9"])
@pytest.mark.parametrize("filter", ["","my_pets"])
...
@pytest.mark.parametrize("filter", ["abracadabra"])
...
@pytest.mark.parametrize("auth_key", ["abracadabra", "e2c4926695071b220431a96f3a1ccd9c0f9e6e0e43e2e0aec70ad1e9"])
@pytest.mark.parametrize("filter", ["qwertyuiop"])
def test_get_list_of_pets_not_valid_all( auth_key, filter):
    auth_key = {'key': auth_key}
